...
{ TSurfaceBitmapPropertyEditor }
  ...
  TSurfaceBitmapPropertyEditor = class(TClassPropertyEditor)
  public
...
begin
  Bitmap := TSurfaceBitmap(GetObjectValue(TSurfaceBitmap));
  if Bitmap <> nil then
  begin
    if EditSurfaceBitmap(Bitmap) then
      Modified;
    if GlobalDesignHook <> nil then
      GlobalDesignHook.AddDependency(Bitmap.ClassType, '');
  end;
end;
...
begin
  ...
  begin
    ...
    if EditRenderImage(Image) then
      Modified;
    if GlobalDesignHook <> nil then
      GlobalDesignHook.AddDependency(Image.ClassType, '');
  end;
end;
