...
{ <include docs/codebot.interop.sockets.txt> }
unit Codebot.Interop.Sockets;
...
{$ifdef darwin}
  {$define libsocket := external 'libSystem.dylib'}
{$endif}
{$ifdef linux}
...
{ Option flags per-socket. }
  ...
  SO_EXCLUSIVEADDRUSE = LongWord(not SO_REUSEADDR);
...
function ioctlsocket(s: TSocketHandle; cmd: LongWord; var arg: LongWord): LongInt; apicall; libsocket;
{$else}
  O_NONBLOCK = 2048;

function fcntl(s: TSocketHandle; cmd, arg: LongInt): LongInt; apicall; libsocket;
{$endif}
...
{$ifdef linux}
function geterrnolocation: PLongInt; apicall; libsocket name '__errno_location';
