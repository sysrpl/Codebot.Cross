...
procedure TIndeterminateProgress.Colorize(Color: TColor);
var
  Images: TImageStrip;
  C: TColorB;
  B: IBitmap;
...
begin
  ...
  begin
    ...
    begin
      ...
      F := A / $FF;
      P.Red := Round(C.Red * F);
      P.Green := Round(C.Green * F);
      P.Blue := Round(C.Blue * F);
      P.Alpha := A;
      Inc(P);
    end;
    Images := FBusyImages;
    ...
    begin
      FBusyImages := TImageStrip.Create(Self);
      FBusyImages.Clear;
    end;
    FBusyImages.Add(B);
  end;
...
var
  ...
  function GetTextRect: TRectI;
  var
    P: TPointF;
  begin
    P := Surface.TextSize(F, S);
    Result := TRectF.Create(Round(P.X), Round(P.Y));
    case FAlignment;
      taLeftJustify: Result.X := Margin
      taCenter: Result.X := ClientWidth div 2 - Result.Width div 2;
      taRightJustify: Result.X := ClientWidth - Result.Width - Margin;
    end;
    case FIconPosition of
      icNear: Result.X := Result.X + Images.Size + Margin;
      icFar: Result.X := Result.X - Images.Size - Margin;
    end;
    Result.Y := ClientHeight div 2 - Result.Height div 2;
    case FIconPosition of
      icAbove: Result.Y := Result.Y + Images.Size div 2 + Margin;
      icBelow: Result.Y := Result.Y - Images.Size div 2 - Margin;
    end;
  end;

  function GetImageRect: TRectI;
  begin
    Result := GetTextRect;
    ...
    case FIconPosition of
      icNear:
        begin
          Result.X := Result.X - Images.Size - Margin;
          Result.Y := Result.Y - (Result.Height );

        end;
      icFar: Result.X := Result.Right + Margin;
      icAbove: Result.Y := Result.Y + Images.Size div 2 + Margin;
      icBelow: Result.Y := Result.Y - Images.Size div 2 - Margin;
    end;
    Result.Width := Images.Size;
    Result.Height := Images.Size;
  end;
...
procedure TIndeterminateProgress.SetAlignment(Value: TAlignment);
begin
